...
function bloblari_kontrol_et($eski_content, $yeni_content){
	$regex = '/\b(qa=)[-A-Z0-9+&@#\/%?=~_|$!:,.;]*[A-Z0-9+&@#\/%=~_|$]/i';
			preg_match_all($regex, $eski_content, $matches);
    foreach($matches[0] as $img) {

        if(strpos($img,'qa_blobid=') !== false)
            {
                $dilimler = explode("qa_blobid=", $img);
                require_once QA_INCLUDE_DIR.'qa-app-blobs.php';
            if(!empty($dilimler[1])) {
				$resim_varmi = strpos($yeni_content, $dilimler[1]);
				if ($resim_varmi==false) qa_delete_blob($dilimler[1]); //qa_fatal_error('silinen resim:'.$dilimler[1]);
			}
            }							   
    }
    return true;  
...
{
	bloblari_kontrol_et($oldquestion['content'],$content);
...
function qa_question_delete($oldquestion, $userid, $handle, $cookieid, $oldclosepost = null)
{
	...
	$oldpath = qa_db_post_get_category_path($oldquestion['postid']);
    bloblari_sil($oldquestion['content']);
...
function qa_answer_delete($oldanswer, $question, $userid, $handle, $cookieid)
{
	...
	bloblari_sil($oldanswer['content']);
...
function bloblari_sil($gelen_content){
    //$regex = '/\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|$!:,.;]*[A-Z0-9+&@#\/%=~_|$]/i';
	$regex = '/\b(qa=)[-A-Z0-9+&@#\/%?=~_|$!:,.;]*[A-Z0-9+&@#\/%=~_|$]/i';
			preg_match_all($regex, $gelen_content, $matches);
    $yazi = "--";
    foreach($matches[0] as $img) {
        if(strpos($img,'qa_blobid=') !== false)
            {
                $dilimler = explode("qa_blobid=", $img);
                $yazi= $yazi . strval($dilimler[1]) . "*****";
                require_once QA_INCLUDE_DIR.'qa-app-blobs.php';
            if(!empty($dilimler[1])) qa_delete_blob($dilimler[1]);
            }						   
    }
    return $yazi;
}
