...
function qa_db_top_users_selectspec($start, $count = null)
{
	...
	if (QA_FINAL_EXTERNAL_USERS) {
		return array(
			...
			'source' => '^userpoints ORDER BY userid DESC LIMIT #,#',
			'arguments' => array($start, $count),
			'arraykey' => 'userid',
			'sortdesc' => 'points',
		);
	...
	if (defined('QA_MYSQL_USERS_PREFIX')) {
		...
		$source = '^users JOIN (SELECT ^users.userid, COALESCE(points,' . $basePoints . ') AS points FROM ^users LEFT JOIN ^userpoints ON ^users.userid=^userpoints.userid ORDER BY userid DESC LIMIT #,#) y ON ^users.userid=y.userid';
	} else {
		$source = '^users JOIN (SELECT userid FROM ^userpoints ORDER BY userid DESC LIMIT #,#) y ON ^users.userid=y.userid JOIN ^userpoints ON ^users.userid=^userpoints.userid';;
	}
...
{
	$source = '^users WHERE (flags & #) ORDER BY WRITTEN DESC';
